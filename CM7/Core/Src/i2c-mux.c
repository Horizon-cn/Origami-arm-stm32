...
void init_BNO(I2C_HandleTypeDef *hi2c, int frequency, i2c_mux_t* mux,int num){
	for (int ch = 0; ch < num; ch++) {
    	   if (i2c_mux_select(mux, ch)) {
		   printf("Failed to select MUX channel %d\r\n", ch);
		   Error_Handler();
	   }
	   HAL_Delay(10);  // Give some time after switching channels
	   send_feature_command(hi2c, frequency);
	   HAL_Delay(500);  // wait for BNO085 to set up the feature;
	}
}
